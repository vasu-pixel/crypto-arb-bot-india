...
TEST_F(PaperExecutorTest, AdverseSlippageWorsensFillPrice) {
  ...
  slip_cfg.slippage_bps_mean = 5.0;
  ...
  for (int i = 0; i < N; ++i) {
    PaperExecutor exec_no(initial, active, aggregator, *fee_manager, *logger,
                          no_slip_cfg);
    PaperExecutor exec_slip(initial, active, aggregator, *fee_manager, *logger,
                            slip_cfg);
  ...
  if (successful > 5) {
    ...
    EXPECT_GT(total_price_slip / successful,
              total_price_no_slip / successful);
  }
}
...
TEST_F(PaperExecutorTest, CompetitionRejectsSomeOpportunities) {
  ...
  EXPECT_GT(rejected, 0)
      << "Expected some competition rejections with 50% base prob";
...
TEST_F(PaperExecutorTest, OneLegRiskCausesFailures) {
  ...
  EXPECT_GT(one_leg_count, 0)
      << "Expected some one-leg failures with 50% probability";
}
