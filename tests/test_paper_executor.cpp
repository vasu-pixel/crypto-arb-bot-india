...
public:
    ...
    std::string exchange_name() const override { return exchange_to_string(id_); }
    ...
    OrderBookSnapshot fetch_order_book(const std::string&, int) override { return {}; }
    std::vector<std::pair<std::string, double>> fetch_top_pairs_by_volume(int) override {
        return {{"BTC-USD", 1000000.0}};
    }
    ...
    OrderResult cancel_order(const std::string&, const std::string&) override { return {}; }
    OrderResult query_order(const std::string&, const std::string&) override { return {}; }
...
protected:
    ...
    StubExchange binance_stub{Exchange::BINANCE_US, 0.001, 0.001};
    StubExchange kraken_stub{Exchange::KRAKEN, 0.0016, 0.0026};
    StubExchange coinbase_stub{Exchange::COINBASE, 0.004, 0.006};
    std::vector<IExchange*> stub_exchanges;
    std::unique_ptr<FeeManager> fee_manager;

    void SetUp() override {
        test_log_file = "/tmp/paper_test_trades_" +
        ...
        stub_exchanges = {&binance_stub, &kraken_stub, &coinbase_stub};
...
TEST_F(PaperExecutorTest, VirtualBalancesInitialized) {
    ...
    std::map<std::string, double> initial = {{"USD", 30000.0}, {"BTC", 0.3}};
    ...
    EXPECT_NEAR(balances[Exchange::BINANCE_US]["USD"], 10000.0, 0.01);
    EXPECT_NEAR(balances[Exchange::BINANCE_US]["BTC"], 0.1, 0.001);
    EXPECT_NEAR(balances[Exchange::KRAKEN]["USD"], 10000.0, 0.01);
    EXPECT_NEAR(balances[Exchange::KRAKEN]["BTC"], 0.1, 0.001);
    EXPECT_NEAR(balances[Exchange::COINBASE]["USD"], 10000.0, 0.01);
    EXPECT_NEAR(balances[Exchange::COINBASE]["BTC"], 0.1, 0.001);
}
...
TEST_F(PaperExecutorTest, ExecuteArbitrageUpdatesBalances) {
    ...
    opp.pair = "BTC-USD";
    opp.buy_exchange = Exchange::BINANCE_US;
    ...
    EXPECT_EQ(record.pair, "BTC-USD");

    // Balances should have changed
    auto balances = executor.get_virtual_balances();

    if (record.buy_result.status == OrderStatus::FILLED) {
        EXPECT_GT(balances[Exchange::BINANCE_US]["BTC"], 0.1);
        EXPECT_LT(balances[Exchange::KRAKEN]["BTC"], 0.1);
    }
}

TEST_F(PaperExecutorTest, InsufficientBalanceRejects) {
    std::map<std::string, double> initial = {{"USD", 3.0}, {"BTC", 0.0}};
    PaperExecutor executor(initial, aggregator, *fee_manager, *logger);
...
TEST_F(PaperExecutorTest, PerExchangeFeesApplied) {
    ...
    // Execute buy on Binance (0.1% taker) and sell on Kraken (0.26% taker)
    ArbitrageOpportunity opp;
    opp.pair = "BTC-USD";
    opp.buy_exchange = Exchange::BINANCE_US;
    opp.sell_exchange = Exchange::KRAKEN;
    ...
    if (record.buy_result.status == OrderStatus::FILLED &&
        ...
        // Buy fee should be less than sell fee (Binance < Kraken)
