...
double PaperExecutor::sample_latency_ms(Exchange /*exch*/) {
  if (!realism_.enable_latency)
    return 0.0;
  // Log-normal distribution parameterized by mean and stddev
  double mu = realism_.latency_mean_ms;
  double sigma = realism_.latency_stddev_ms;
  if (sigma <= 0.0)
    return mu;
  // Convert desired mean/stddev to log-normal parameters
...
PaperExecutor::apply_phantom_to_snapshot(const OrderBookSnapshot &snap,
  ...
  for (const auto &f : recent_fills_) {
    if (f.exchange == exch && f.pair == pair && f.side == side) {
      ...
      consumed += f.quantity * std::exp(-elapsed / realism_.impact_decay_seconds);
    }
  }
  if (consumed <= 0.0)
    return snap;

  // Clone and reduce liquidity
...
void PaperExecutor::settle_pending_transfers() {
  ...
  while (it != pending_transfers_.end()) {
    if (now >= it->available_at) {
      ...
      LOG_INFO("[PAPER] Transfer settled: {:.6f} {} → {}",
               it->amount, it->asset, exchange_to_string(it->to_exchange));
...
TradeRecord PaperExecutor::execute(const ArbitrageOpportunity &opp) {
  ...
  if (realism_.enable_staleness_penalty) {
    ...
    double buy_age_ms =
        std::chrono::duration<double, std::milli>(now - buy_snap.local_timestamp)
            .count();
    double sell_age_ms =
        std::chrono::duration<double, std::milli>(now - sell_snap.local_timestamp)
            .count();
  ...
  buy_snap =
      apply_phantom_to_snapshot(buy_snap, opp.buy_exchange, opp.pair, Side::BUY);
  ...
  record.buy_result =
      simulate_fill(buy_req, buy_snap, buy_taker_fee, buy_staleness_bps,
                    opp.net_spread_bps);
  record.sell_result =
      simulate_fill(sell_req, sell_snap, sell_taker_fee, sell_staleness_bps,
                    opp.net_spread_bps);
  ...
  if (realism_.enable_one_leg_risk) {
    ...
    if (u(rng_) < realism_.one_leg_probability) {
      ...
      if (buy_fails) {
        ...
        update_virtual_balance(opp.buy_exchange, quote, -unwind_cost);
      }
...
void PaperExecutor::rebalance() {
  ...
  for (auto &[asset, total] : totals) {
    ...
    for (auto &[exch, assets] : virtual_balances_) {
      ...
      if (excess <= 0.0)
        continue; // This exchange needs funds, not sends them
      ...
      if (realism_.enable_withdrawal_fees) {
        ...
        double pct_fee = amount_to_transfer * realism_.withdrawal_fee_pct / 100.0;
        amount_to_transfer -= (flat_fee + pct_fee);
        if (amount_to_transfer <= 0.0) {
          LOG_WARN("[PAPER] Withdrawal fees exceed transfer amount for {}", asset);
          continue;
        }
      }

      // Distribute the net amount to deficit exchanges via pending transfers
      for (auto &[dest_exch, dest_assets] : virtual_balances_) {
        ...
        double transfer_share =
            std::min(deficit, amount_to_transfer);
