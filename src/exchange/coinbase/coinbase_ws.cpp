...
CoinbaseWs::CoinbaseWs(const std::string &ws_base_url,
                       ExchangeWsClient &ws_client, CoinbaseAuth &auth)
...
void CoinbaseWs::on_connected() {
  ...
  nlohmann::json heartbeat_sub = {{"type", "subscribe"},
  ...
  nlohmann::json level2_sub = {{"type", "subscribe"},
                               {"product_ids", pending_subs_},
                               {"channel", "level2"},
                               {"jwt", jwt}};
  ws_client_.send(level2_sub.dump());
  LOG_INFO("Coinbase: sent batched subscription for {} products",
           pending_subs_.size());
}

void CoinbaseWs::subscribe_level2(const std::string &product_id,
...
void CoinbaseWs::unsubscribe_level2(const std::string &product_id) {
  std::string jwt = auth_.generate_jwt("GET", "/ws");
  nlohmann::json unsub = {{"type", "unsubscribe"},
                          {"product_ids", {product_id}},
                          {"channel", "level2"},
                          {"jwt", jwt}};
