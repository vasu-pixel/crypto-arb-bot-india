...
CoinbaseAdapter::CoinbaseAdapter(const Config& config) {
    ...
    if (it == config.exchanges.end()) {
        throw std::runtime_error("CoinbaseAdapter: no exchange config for COINBASE");
    }
    config_ = it->second;
...
std::string CoinbaseAdapter::normalize_pair(const std::string& canonical_pair) const {
    return canonical_pair;
}

std::string CoinbaseAdapter::canonical_pair(const std::string& native_pair) const {
    return native_pair;
}

OrderBookSnapshot CoinbaseAdapter::fetch_order_book(const std::string& pair, int depth) {
    return rest_->fetch_product_book(pair, depth);
}

std::vector<std::pair<std::string, double>> CoinbaseAdapter::fetch_top_pairs_by_volume(int limit) {
    auto products = rest_->fetch_products();
    // Filter USD pairs
    std::vector<std::pair<std::string, double>> result;
    for (auto& [id, volume] : products) {
        if (id.find("-USD") != std::string::npos && id.find("-USDT") == std::string::npos) {
...
OrderResult CoinbaseAdapter::cancel_order(const std::string& pair, const std::string& order_id) {
    return rest_->cancel_order(order_id);
}

OrderResult CoinbaseAdapter::query_order(const std::string& pair, const std::string& order_id) {
...
void CoinbaseAdapter::subscribe_order_book(const std::string& pair, OrderBookCallback cb) {
