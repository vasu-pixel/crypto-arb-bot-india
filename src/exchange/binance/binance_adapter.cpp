...
BinanceAdapter::BinanceAdapter(const Config& config) {
    ...
    if (it == config.exchanges.end()) {
        throw std::runtime_error("BinanceAdapter: no exchange config for BINANCE_US");
    }
    config_ = it->second;
...
std::string BinanceAdapter::normalize_pair(const std::string& canonical_pair) const {
...
std::string BinanceAdapter::canonical_pair(const std::string& native_pair) const {
...
OrderBookSnapshot BinanceAdapter::fetch_order_book(const std::string& pair, int depth) {
    return rest_->fetch_order_book(normalize_pair(pair), depth);
}

std::vector<std::pair<std::string, double>> BinanceAdapter::fetch_top_pairs_by_volume(int limit) {
...
OrderResult BinanceAdapter::cancel_order(const std::string& pair, const std::string& order_id) {
    return rest_->cancel_order(normalize_pair(pair), order_id);
}

OrderResult BinanceAdapter::query_order(const std::string& pair, const std::string& order_id) {
...
void BinanceAdapter::subscribe_order_book(const std::string& pair, OrderBookCallback cb) {
