...
std::string
KrakenAdapter::normalize_pair(const std::string &canonical_pair) const {
  auto it = rest_pair_map_.find(canonical_pair);
  if (it != rest_pair_map_.end())
    return it->second;
...
std::string
KrakenAdapter::canonical_pair(const std::string &native_pair) const {
  // Check REST map
  ...
  for (auto &[canonical, ws_name] : ws_pair_map_) {
    if (ws_name == native_pair)
      return canonical;
  }
  // Heuristic for "BTC/USD" format
  auto slash_pos = native_pair.find('/');
  if (slash_pos != std::string::npos) {
    return native_pair.substr(0, slash_pos) + "-" +
           native_pair.substr(slash_pos + 1);
  }
  return native_pair;
}

OrderBookSnapshot KrakenAdapter::fetch_order_book(const std::string &pair,
                                                  int depth) {
...
KrakenAdapter::fetch_top_pairs_by_volume(int limit) {
...
OrderResult KrakenAdapter::cancel_order(const std::string &pair,
                                        const std::string &order_id) {
  return rest_->cancel_order(order_id);
}

OrderResult KrakenAdapter::query_order(const std::string &pair,
                                       const std::string &order_id) {
...
void KrakenAdapter::subscribe_order_book(const std::string &pair,
                                         OrderBookCallback cb) {
  auto it = ws_pair_map_.find(pair);
  std::string ws_pair = (it != ws_pair_map_.end()) ? it->second : pair;

  ws_->subscribe_depth(ws_pair,
                       [this, pair, cb](const OrderBookSnapshot &snap) {
...
bool KrakenAdapter::is_connected() const { return ws_client_->is_connected(); }
