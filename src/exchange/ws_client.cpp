...
ExchangeWsClient::ExchangeWsClient(const std::string& uri)
    : uri_(uri) {
    ...
    client_.set_open_handler(
        [this](ConnectionHdl hdl) { on_open(hdl); });
    client_.set_close_handler(
        [this](ConnectionHdl hdl) { on_close(hdl); });
    client_.set_fail_handler(
        [this](ConnectionHdl hdl) { on_fail(hdl); });
    client_.set_message_handler(
        [this](ConnectionHdl hdl, MessagePtr msg) { on_message(hdl, msg); });
    client_.set_ping_handler(
        [this](ConnectionHdl hdl, std::string payload) { return on_ping(hdl, payload); });
...
ExchangeWsClient::~ExchangeWsClient() {
    disconnect();
}

// ─── Callback setters ───────────────────────────────────────────────────────

void ExchangeWsClient::set_on_message(std::function<void(const std::string&)> callback) {
...
void ExchangeWsClient::disconnect() {
    ...
    if (connected_.load()) {
        websocketpp::lib::error_code ec;
        std::lock_guard<std::mutex> lock(connection_mutex_);
        client_.close(connection_hdl_, websocketpp::close::status::normal, "client disconnect", ec);
        if (ec) {
...
bool ExchangeWsClient::is_connected() const {
...
ExchangeWsClient::SslContext ExchangeWsClient::on_tls_init(ConnectionHdl /*hdl*/) {
...
void ExchangeWsClient::reconnect_loop() {
    while (!shutting_down_.load() && auto_reconnect_.load() && !connected_.load()) {
        ...
        auto con = client_.get_connection(uri_, ec);
        if (ec) {
            LOG_ERROR("ExchangeWsClient: reconnect get_connection error: {}", ec.message());
            continue;
        }
