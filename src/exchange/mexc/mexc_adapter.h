...
class MexcAdapter : public IExchange {
public:
    explicit MexcAdapter(const Config& config);
    ~MexcAdapter() override = default;

    Exchange exchange_id() const override { return Exchange::MEXC; }
    std::string exchange_name() const override { return "MEXC"; }
...
private:
    ...
    std::unique_ptr<MexcAuth> auth_;
    std::unique_ptr<MexcRest> rest_;
    std::unique_ptr<ExchangeWsClient> ws_client_;
    std::unique_ptr<MexcWs> ws_;
