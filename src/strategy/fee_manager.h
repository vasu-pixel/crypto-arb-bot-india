...
public:
    ...
    double total_fee_rate(Exchange buy_exch, Exchange sell_exch, const std::string& pair) const;

    // Launch a background thread that calls refresh_all_fees every `interval`.
    void start_periodic_refresh(std::chrono::seconds interval = std::chrono::seconds(3600));
...
private:
    // Key: "EXCHANGE:PAIR"
    std::unordered_map<std::string, FeeInfo> fee_cache_;
    mutable std::shared_mutex mutex_;
    std::vector<IExchange*> exchanges_;
    std::thread refresh_thread_;
    std::atomic<bool> running_{false};

    std::string make_key(Exchange exch, const std::string& pair) const;
};
