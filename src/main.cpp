...
std::vector<std::string> discover_top_pairs(
    std::unordered_map<Exchange, IExchange*>& exchanges, int limit) {
    ...
    for (auto& [exch, adapter] : exchanges) {
        try {
            ...
            LOG_ERROR("Failed to fetch pairs from {}: {}", exchange_to_string(exch), e.what());
        }
    }
    ...
    for (int i = 0; i < limit && i < static_cast<int>(candidates.size()); ++i) {
        ...
        LOG_INFO("Monitoring pair: {} (avg volume: {:.0f})", candidates[i].first, candidates[i].second);
    }
    return result;
}
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (arg == "--backtest") run_backtest = true;
        else if (arg == "--from" && i + 1 < argc) backtest_from = argv[++i];
        else if (arg == "--to" && i + 1 < argc) backtest_to = argv[++i];
    }
    ...
    LOG_INFO("Crypto Arb Bot v1.0.0 starting in {} mode", trading_mode_to_string(config.mode));
    ...
    std::vector<IExchange*> exch_ptrs = {binance.get(), kraken.get(), coinbase.get()};
    ...
    for (auto& [exch, adapter] : exchanges) {
        for (auto& pair : pairs) {
            ...
            adapter->subscribe_order_book(pair, [&book](const OrderBookSnapshot& snap) {
    ...
    std::string trades_file = (config.mode == TradingMode::PAPER) ?
                               config.paper_trades_file : config.trades_file;
    ...
    // Spread detector
    SpreadDetector detector(aggregator, fee_manager,
    ...
    detector.set_opportunity_callback([&](const ArbitrageOpportunity& opp) {
        LOG_INFO("Opportunity: {} buy@{}({}) sell@{}({}) net={}bps",
    ...
    while (!g_shutdown) {
        ...
        if (config.mode == TradingMode::PAPER && paper_executor) {
            ...
            for (auto& [exch, assets] : vb) balances_map[exch] = assets;
            ws_server.broadcast_balances(balances_map);
        }

        // Broadcast P&L
        double total_pnl = (config.mode == TradingMode::PAPER && paper_executor) ?
                            paper_executor->get_virtual_pnl() : trade_logger.total_realized_pnl();
