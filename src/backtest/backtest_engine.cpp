...
BacktestMetrics
BacktestEngine::run(const std::vector<HistoricalSnapshot> &data) {
  ...
  for (auto &ef : exch_fees) {
    sim_exchanges[ef.id] = std::make_unique<SimulatedExchange>(
        ef.id, std::string(exchange_to_string(ef.id)), config_.initial_balances,
        ef.maker, ef.taker);
    ...
    for (auto &pair : active_pairs) {
      ...
      for (size_t i = 0; i < snapshots.size(); ++i) {
        for (size_t j = 0; j < snapshots.size(); ++j) {
          ...
          auto eff_sell =
              DepthAnalyzer::effective_sell_price(sell_snap, quantity);

          if (!eff_buy.fully_fillable || !eff_sell.fully_fillable)
            continue;
          ...
          if (net_spread_bps < config_.min_net_spread_bps)
            continue;

          // Execute on simulated exchanges
          OrderRequest buy_req{buy_snap.exchange, pair,
                               Side::BUY,         eff_buy.avg_price * 1.001,
                               quantity,          CryptoUtils::generate_uuid()};
          OrderRequest sell_req{
              sell_snap.exchange, pair,
              Side::SELL,         eff_sell.avg_price * 0.999,
              quantity,           CryptoUtils::generate_uuid()};

          auto buy_result =
              sim_exchanges[buy_snap.exchange]->place_limit_order(buy_req);
          auto sell_result =
              sim_exchanges[sell_snap.exchange]->place_limit_order(sell_req);
          ...
          double matched_qty =
              std::min(buy_result.filled_quantity, sell_result.filled_quantity);
          record.realized_pnl =
              (sell_result.avg_fill_price - buy_result.avg_fill_price) *
                  matched_qty -
