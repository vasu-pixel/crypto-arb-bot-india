...
std::vector<HistoricalSnapshot> DataLoader::load_csv(const std::string& filepath) {
    ...
    while (std::getline(file, line)) {
        ...
        // Format: timestamp_ms,exchange,pair,bid1_price,bid1_qty,...,ask1_price,ask1_qty,...
        std::getline(ss, token, ','); snap.timestamp_ms = std::stoull(token);
        std::getline(ss, token, ',');
        snap.exchange = exchange_from_string(token);
        std::getline(ss, token, ','); snap.pair = token;

        // Read up to 20 bid levels
        for (int i = 0; i < 20; ++i) {
            std::string price_str, qty_str;
            if (!std::getline(ss, price_str, ',')) break;
            if (!std::getline(ss, qty_str, ',')) break;
            if (price_str.empty()) break;
            double price = std::stod(price_str);
            double qty = std::stod(qty_str);
            if (price > 0) snap.bids.push_back({price, qty});
        }
        ...
        for (int i = 0; i < 20; ++i) {
            ...
            double price = std::stod(price_str);
            double qty = std::stod(qty_str);
            if (price > 0) snap.asks.push_back({price, qty});
        }
...
std::vector<HistoricalSnapshot> DataLoader::load_csv_directory(const std::string& directory) {
...
void DataLoader::save_snapshot_csv(const std::string& directory,
    ...
    std::string filename = directory + "/" +
                           exchange_to_string(snapshot.exchange) + "_" +
                           snapshot.pair + ".csv";
    ...
    if (!file_exists) {
        ...
        for (int i = 0; i < 20; ++i) file << ",bid" << i << "_price,bid" << i << "_qty";
        for (int i = 0; i < 20; ++i) file << ",ask" << i << "_price,ask" << i << "_qty";
        file << "\n";
    }

    file << timestamp_ms << "," << exchange_to_string(snapshot.exchange) << "," << snapshot.pair;
    for (int i = 0; i < 20; ++i) {
...
std::vector<HistoricalSnapshot> DataLoader::filter(
    const std::vector<HistoricalSnapshot>& data,
    uint64_t from_ms, uint64_t to_ms,
    const std::vector<std::string>& pairs) {
    std::vector<HistoricalSnapshot> result;
    for (auto& snap : data) {
        if (snap.timestamp_ms < from_ms || snap.timestamp_ms > to_ms) continue;
        if (!pairs.empty()) {
            bool found = false;
            for (auto& p : pairs) {
                if (snap.pair == p) { found = true; break; }
            }
            if (!found) continue;
        }
